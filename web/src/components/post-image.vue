  ...
  <div class="images-wrap">
    <n-image-group v-if="[1].includes(props.imgs.length)">
      <n-grid :x-gap="4" :y-gap="4" :cols="2">
        <template v-for="img in props.imgs" :key="img.id">
          <n-gi>
            <n-image @error="() => (img.content = defaultImg)" @click.stop class="post-img x1" object-fit="cover"
              :src="img.content + thumbnail" :preview-src="img.content" />
          </n-gi>
        </template>
      </n-grid>
    </n-image-group>
    ...
    <n-image-group v-if="[2, 3].includes(props.imgs.length)">
      <n-grid :x-gap="4" :y-gap="4" :cols="3">
        <template v-for="img in props.imgs" :key="img.id">
          <n-gi>
            <n-image @error="() => (img.content = defaultImg)" @click.stop class="post-img x2" object-fit="cover"
              :src="img.content + thumbnail" :preview-src="img.content" />
          </n-gi>
        </template>
      </n-grid>
    ...
    <n-image-group v-if="[4].includes(props.imgs.length)">
      <n-grid :x-gap="4" :y-gap="4" :cols="4">
        <template v-for="img in props.imgs" :key="img.id">
          <n-gi>
            <n-image @error="() => (img.content = defaultImg)" @click.stop class="post-img x3" object-fit="cover"
              :src="img.content + thumbnail" :preview-src="img.content" />
          </n-gi>
        </template>
      </n-grid>
...
const defaultImg = notfound; //import.meta.env.VITE_DEFAULT_TWEET_IMAGE_404;
